...
export interface ConfirmDialogProps extends HTMLAttributes<HTMLDivElement> {
  ...
  message?: string
  /** Detailed description (supports multiline with \n) */
  description?: string
  /** Confirm button text */
  confirmText?: string
  /** Cancel button text */
...
export function ConfirmDialog({
  ...
  // Determine variant based on isDangerous or explicit variant
  const variant = isDangerous ? 'danger' : (variantProp || 'primary')
  ...
  // Render message/description with line break support
  const messageContent = description ? description.split('\n').map((line, idx) => (
      ...
      >
        ...
        <div className="p-6 text-center">
          ...
          <div
            ...
            className={`text-base mb-6 space-y-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}
          >
            {messageContent}
          </div>
