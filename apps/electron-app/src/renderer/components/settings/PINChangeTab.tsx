      ...
      {currentUser?.recovery_pin && (
        ...
        `}>
          <div className="flex items-start gap-3">
            <svg className={`w-6 h-6 flex-shrink-0 ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
            <div className="flex-1">
              <h3 className={`text-lg font-semibold mb-2 ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-800'}`}>
                Recovery PIN
              </h3>
              <p className={`text-sm mb-3 ${theme === 'dark' ? 'text-yellow-300' : 'text-yellow-700'}`}>
                Store this recovery PIN in a safe place. You can use it to recover your account if you forget your main PIN.
              </p>
              <div className={`
                p-4 rounded-lg font-mono text-2xl tracking-widest text-center
                ${theme === 'dark'
              ...
              <p className={`text-xs mt-2 ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600'}`}>
                ⚠️ Keep this PIN secure and do not share it with anyone
              </p>
            </div>
          </div>
        </div>
